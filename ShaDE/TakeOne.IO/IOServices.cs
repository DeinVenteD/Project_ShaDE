...
namespace TakeOne.IO
{
    ...
    public class SwitchSourceStream
    {
        ...
        public unsafe double ReadVAXPDPDouble()
        {
            /*
            long Value;
            fixed (byte* PointerByte = &Bytes[Index])
            {
                int i1 = (*(PointerByte + 1) - 1 * (*(PointerByte + 1) == 0 ? 0 : 1)) << 24 | (*(PointerByte) << 16) | (*(PointerByte + 3) << 8) | (*(PointerByte + 2));
                int i2 = (*(PointerByte + 5)) << 16 | (*(PointerByte + 4) << 24) | (*(PointerByte + 7) << 8) | (*(PointerByte + 6));
                Value = (uint)i1 | ((long)i2 << 32); //ne valja
            }*/
            Index += 8;
            return Double.NaN;//*(double*)&Value;
        }
        public void Close()
        {
            this.Bytes = null;
    ...
    public static class Parser
    {
        ...
        public static string MustParse(StreamReader Stream)
        {
            string Parsed = "";
        ...
        public static float ParseSingle(StreamReader Stream)
        {
            ...
            while (true)
            {
                ...
                if (NC == ' ' || NC == '\t' || NC == '\n' || NC == '\r')
                {
                    return SingleParse(NewString);
                }
                else NewString += NC;
            }
        }
        public static string ParseDoubleString(StreamReader Stream)
        {
            if (Stream.EndOfStream) return "";
            string NewString = "";
            while (true)
        ...
        public static double ReadDouble(StreamReader Reader)
        {
            ...
            return DoubleParse(Parsed);
        }
        public static float ReadSingle(StreamReader Reader)
        {
            string Parsed = "";
            while (Parsed == "") Parsed = Parse(Reader);
            return SingleParse(Parsed);
        }
        ...
        public static double[] ParseDoubles(StreamReader Reader)
        {
            ...
            while (true)
            {
                Current = ParseDoubleString(Reader);
        ...
        private static float SingleParse(string Input)
        {
    ...
    {
        public static float ToDECFloat(byte[] Read)
        {
